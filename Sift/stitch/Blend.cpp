...
static Blend_Image sub(Blend_Image, Blend_Image);
static Blend_Image add(Blend_Image, Blend_Image);
static vector<struct X_S> getXsize(Blend_Image);

vector<bool> buildLaplacianMap(Raw &, vector<Blend_Image> &, int, int, int);
vector<float> getGaussianKernel_rr(int, int, int, int);
void blendImg(Raw &, Blend_Image &, int, int, int, vector<bool> &, vector<bool> &);
...
static Blend_Image Bicubic(Blend_Image& src, float TransMat[3][3])
{
	Blend_Image dst;
...
Blend_Image BlurImage_GG(Blend_Image &img, float sigma, int r) {
	...
	vector<float> img_ori(w*h);
	for(size_t i = 0; i < img_ori.size(); i++) {
		img_ori[i] = img.RGB[i];
	}

	vector<float> img_gau;
	Gaus::GauBlur(img_gau, img_ori, w, h, sigma, (r*2 + 1));
...
//----------------------------------------
// 建立寬度大小個高斯kernel
vector<float> getGaussianKernel(int x)
{
	//----------------------------------------
	// sigma 由半徑去計算
	int i = 0;
	...
	float sigma = sqrt((-1.f) * pow((float)x - 1.f - half, 2.f) / (2.f * log(0.5f)));
	float g = 0.f;
	//----------------------------------------
	for (i = 0; i < x; i++)
	{
...
//----------------------------------------
// 兩圖層相減
Blend_Image sub(Blend_Image m, Blend_Image n)
{
	Blend_Image out;
	out.width = m.width;
	out.height = m.height;
...
//----------------------------------------
// 兩圖層相加
Blend_Image add(Blend_Image m, Blend_Image n)
{
	Blend_Image out;
	out.width = m.width;
	out.height = m.height;
	...
	for (i = 0; i < out.height; i++)
	{
		out_RGB = &out.RGB[i * out.width * 3];
		m_RGB = &m.RGB[i * m.width * 3];
		n_RGB = &n.RGB[i * n.width * 3];
...
//----------------------------------------
// 計算每個高度裡的X寬度
vector<struct X_S> getXsize(Blend_Image input)
{
...
// todo 這個裡面的 BlurImage_GG 要重寫cpu版本的.
vector<bool> buildLaplacianMap(Raw &inputArray, vector<Blend_Image> &outputArrays, int dx, int dy, int lr)
{
	vector<bool> bo;
	Blend_Image tmp;
	tmp.height = abs(dy);
	...
	for(int i = 1; i < PYR_OCTAVE; i++) {
		outputArrays[i] = Bicubic(BlurImage_GG(outputArrays[i - 1], sigma, PYR_R), transMat_0_5);
	...
	for(int a = 0; a < PYR_OCTAVE - 1; a++) {
		outputArrays[a] = sub(outputArrays[a], Bicubic(outputArrays[a + 1], transMat_2));
	}
...
vector<float> getGaussianKernel_rr(int x, int y, int dx, int dy = 0)
{
...
void blendImg(Raw &inputArray, Blend_Image &overlap_area, int dx, int dy, int lr, vector<bool> &bol, vector<bool> &bor)
{
	...
	{
		for (i = 0; i < overlap_area.width - 1; i++)
		{
			if (bol[j * overlap_area.width + i] == true && bor[j * overlap_area.width + i] == true 
			 && bol[j * overlap_area.width + (i + 1)] == true && bor[j * overlap_area.width + (i + 1)] == true)
			{
				if (j + disy < inputArray.getRow() && i + disx < inputArray.getCol())
				{
					/*if (flag == false)
					...
					for (c = 0; c < 3; c++)
					{
						inputArray.RGB[((j + disy) * inputArray.getCol() + (i + disx)) * 3 + c] = (unsigned char)(overlap_area.RGB[(j * overlap_area.width + i) * 3 + c] < 0.f ? 0 : overlap_area.RGB[(j * overlap_area.width + i) * 3 + c] > 255.f ? 255 : overlap_area.RGB[(j * overlap_area.width + i) * 3 + c]);
					}
				}
			}
		}
